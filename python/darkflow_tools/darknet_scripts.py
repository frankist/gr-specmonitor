...
def anchors_fmt_str(centroids,model_width,model_height,separator='\n'):
    ...
    for i in range(anchors.shape[0]):
        anchors[i][0]*=model_width/32.
        anchors[i][1]*=model_height/32.
    ...
    for a in anchors_sorted:
        anchor_str += '%0.2,%0.2f'.format(a[i,0],a[i,1]) + separator
    return anchor_str
...
def compute_kmeans_centroids(annotation_dims,num_anchors,num_attempts=10):
    ...
    assert len(possible_dims)>=num_anchors
    best_dist = np.inf
    best_centroids = []
    eps = 0.005
    for trial in range(num_attempts):
        ...
        # use kmeans to find the write cluster config
...
def read_annotation_dims(imglist_file,darknet_annotation_path):
    ...
    with open(imglist_file) as f:
        for line in f.readlines():
            ...
            darknet_annotation_file = img_id + '.txt'
...
def generate_anchors(imglist_file,darknet_annotations_path,num_anchors):
    ...
    annotation_dims = read_annotation_dims(imglist_file)

    centroids_list = []
    for a in num_anchors:

        centroids,dist = compute_kmeans_centroids(annotation_dims,num_anchors)
        centroids.append((centroids,dist))
        print "Centroids = ",centroids
    return centroids_list

def write_anchors_from_params_dict(cfg_obj):
    '''
    This function generates the best fit for the darknet model
    based on my dataset
    '''
    imglist_filename = cfg_obj.tmp_imgpaths_filename()
    num_anchors = len(cfg_obj.model_params()['num_anchors'])
    ...
    for centroids,dist in centroids_list:
        ...
        write_anchors_to_file(centroids,dist,anchor_file)
